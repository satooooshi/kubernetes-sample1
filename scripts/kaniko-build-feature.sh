...
if [ "${BRANCH:-x}" = 'x' ]; then
  echo "branch name unknown; use tag name ${tag:=feat}"
else
  echo "tag name specified from last path segment of branch name: ${tag:=${BRANCH##*/}}"
fi
export tag
...
if [ "${REPOSITORY:-x}" = 'x' ]; then
  ...
  destination_opt="--destination=${REPOSITORY}${component}:${tag}"
